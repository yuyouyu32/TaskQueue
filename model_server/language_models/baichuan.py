...
class BaichuanLLM(BaseLLM):
    def __init__(self, path: str, ckpt_path: str = None):
        # default params
        if ckpt_path:
            self.model, self.tokenizer = self._get_model_from_ckpt(path, ckpt_path)
        else:
            self.model, self.tokenizer = self._get_model(path)

    def _get_model_from_ckpt(self, path: str, ckpt_path: str, load_8bit: bool=False):
    ...
    def predict(self, prompt: str, system_prompt: str, history: List[Tuple[str, str]], generation_config: dict = None) -> str:
        ...
        """
        instruction = self.constructe_messages(prompt, system_prompt, history)
        if generation_config:
            for key, value in generation_config.items():
                setattr(self.model.generation_config, key, value)
        response = self.model.chat(self.tokenizer, instruction)
        return response
